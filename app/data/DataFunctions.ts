...
export async function getUsersData() {
  const rawMockUsers = await fs.readFile("./mock.json", "utf-8");
...
export async function getProductsData() {
  const rawProduct = await fs.readFile("./products.json", "utf-8");
  const jProduct = JSON.parse(rawProduct);
  const product = jProduct ?? [];
  return product;
}
export async function getValidazioni() {
  const rawValidazioni = await fs.readFile("./validazioni.json", "utf-8");
